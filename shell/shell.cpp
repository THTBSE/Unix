/* 
 * fclsh - A tiny shell program with job control
 * 
 */
#include <stdio.h>
#include <stdlib.h>
...
char prompt[] = "fclsh$ ";    /* command line prompt (DO NOT CHANGE) */
		...
		else
		{
			for (int i=0; i<num; i++)
			{
				...
				for (int j=0; j<MAXJOBS; j++)
				{
					if (jobs[j].state == ST)
					{
						if (usejid && jobs[j].jid == bgid[i]
							|| jobs[j].pid == bgid[i])
						{
							if (kill(jobs[j].pid, SIGCONT) < 0)
							{
								perror("kill");
								exit(0);
							}
